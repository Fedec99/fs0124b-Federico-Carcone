...
export class UsersService {
  ...
  getAllUsersWithTodos(): void {
    const usersWithTodos: iUser[] = this.UsersArr.map(user => {
      const todos = this.todoService.getUserTodos(user.id);
      return {
        ...user,
        todos: todos
      };
    });
    this.usersSubject.next(usersWithTodos);
  }
}
