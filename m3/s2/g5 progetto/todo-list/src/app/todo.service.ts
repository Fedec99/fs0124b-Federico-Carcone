...
export class TodoService {
  ...
  getAllTodoWithUsers(): { todo: iTodo, user: iUser }[] {
    return this.todoArr.map(todo => ({
      todo: todo,
      user: this.usersService.getUserById(todo.userId) || { id: 0, firstName: 'Unknown', lastName: 'User', email: '', image: '', title: '' }
    }));
  }
